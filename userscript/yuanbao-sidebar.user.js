...
(function() {
    ...
    class YuanbaoSidebarUserScript {
        ...
        isSubmitButton(element) {
            ...
            return buttonSelectors.some(selector => 
        ...
        extractLatestQuestion() {
            // 尝试从输入框获取刚提交的内容
            const inputElements = document.querySelectorAll('textarea, input[type="text"], [contenteditable="true"]');
        ...
        exportHistory() {
            ...
            a.href = url;
            a.download = `yuanbao-questions-${new Date().toISOString().split('T')[0]}.json`;
                ...
                reader.onload = (e) => {
                    try {
                        ...
                        if (data.questions && Array.isArray(data.questions)) {
                            if (confirm(`确定要导入 ${data.questions.length} 条历史记录吗？这将覆盖现有记录。`)) {
